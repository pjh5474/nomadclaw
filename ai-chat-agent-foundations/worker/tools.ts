...
export const getWeather = tool({
	...
	inputSchema: z.object({
		city: z.string().meta({
			description:
				"The name of the city you want to get weather from (ie: Malaga)",
		}),
	}),
	execute: ({ city }) => {
...
export const buyPlaneTicket = tool({
	...
	inputSchema: z.object({
		ticketCode: z.string().meta({
			description: "The ticket code that you want to buy",
		}),
		price: z.number().meta({
	...
	execute: async ({ price, ticketCode }) => {
		return `You have bought the ticket ${ticketCode} for ${price} dollars.`;
	},
